...
/// Admin Export Page
/// Exports expenses with filters applied from the Expenses page
...
class _AdminExportPageState extends State<AdminExportPage> {
  ...
  Future<List<Expense>> _getFilteredExpenses() async {
    ...
    // Apply user filter (admin can filter by user)
  ...
  Widget _buildActiveFiltersCard(AppLocalizations l10n) {
    ...
    final hasFilters = currencyFilter != ExpenseCurrencyFilter.all ||
        ...
        (userFilter != null && userFilter.isNotEmpty);
  ...
  Future<void> _exportToPdf() async {
    ...
    try {
      ...
      // Helper function to wrap text with directionality for Arabic support
      ...
      doc.addPage(
        pw.MultiPage(
          ...
          build: (ctx) => [
            pw.Header(
              level: 0,
              child: _arabicText(l10n.expensesList, fontSize: 18, bold: true),
            ),
            pw.Table(
                ...
                pw.TableRow(
                  ...
                  children: [
                    pw.Padding(
                      ...
                      child: _arabicText(l10n.description, fontSize: 12, bold: true),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: _arabicText('USD', fontSize: 12, bold: true),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: _arabicText('SYP', fontSize: 12, bold: true),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: _arabicText('TRY', fontSize: 12, bold: true),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: _arabicText(l10n.invoice, fontSize: 12, bold: true),
                    ),
                  ],
                ),
                ...expenses.map((e) {
                  final invoiceText = e.invoiceStatus == InvoiceStatus.invoiceAvailable ? l10n.yes : l10n.no;
                  return pw.TableRow(
                    children: [
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: _arabicText(e.description),
                      ),
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: _arabicText(e.priceUsd?.toStringAsFixed(2) ?? '-'),
                      ),
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: _arabicText(e.priceSyp?.toStringAsFixed(0) ?? '-'),
                      ),
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: _arabicText(e.priceTry?.toStringAsFixed(2) ?? '-'),
                      ),
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: _arabicText(invoiceText),
                      ),
                    ],
                  );
                }),
            ...
            pw.Header(
              level: 1,
              child: _arabicText(l10n.summary, fontSize: 16, bold: true),
            ),
            pw.SizedBox(height: 10),
            _arabicText(
              '${l10n.totalUsd}: ${expenses.fold(0.0, (sum, e) => sum + (e.priceUsd ?? 0)).toStringAsFixed(2)}',
              fontSize: 12,
            ),
            _arabicText(
              '${l10n.totalSyp}: ${expenses.fold(0.0, (sum, e) => sum + (e.priceSyp ?? 0)).toStringAsFixed(0)}',
              fontSize: 12,
            ),
            _arabicText(
              '${l10n.totalTry}: ${expenses.fold(0.0, (sum, e) => sum + (e.priceTry ?? 0)).toStringAsFixed(2)}',
              fontSize: 12,
            ),
          ],
        ),
